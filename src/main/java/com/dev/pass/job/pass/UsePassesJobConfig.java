package com.dev.pass.job.pass;

import com.dev.pass.repository.booking.BookingEntity;
import com.dev.pass.repository.booking.BookingRepository;
import com.dev.pass.repository.booking.BookingStatus;
import com.dev.pass.repository.pass.PassEntity;
import com.dev.pass.repository.pass.PassRepository;
...
public class UsePassesJobConfig {
    ...
    public JpaCursorItemReader<BookingEntity> usePassesItemReader() {
        return new JpaCursorItemReaderBuilder<BookingEntity>()
                ...
                // 상태(status)가 완료이며, 종료 일시(endedAt)이 과거인 예약이 이용권 차감 대상
    ...
    /**
     * 이 프로젝트에서는 적합하지 않지만, ItemProcessor의 수행이 오래걸려 병목이 생기는 경우에
     *  AsyncItemProcessor, AsyncItemWriter를 사용하면 성능을 향상시킬 수 있다!
     *
     * 즉, AsyncItemProcessor 내부에서 delegate로 itemProcessor 를 설정해주면
     * 데이터가 유입됐을 때 새로운 Thread 를 띄워서 ItemProcessor 에게 위임하는 동작 방식
     */
    @Bean
    ...
    public AsyncItemWriter<BookingEntity> usePassesAsyncItemWriter() {
        ...
        // usePassesItemWriter 최종 결과값을 넘겨주고 작업을 위임!
